...
**根本原因：** py2app 打包后的环境中，`NSStatusBarButton.setTitle_()` 设置的纯文字标题不会在菜单栏中显示，但 `NSStatusBarButton.setImage_()` 设置的图标可以正常显示。

**解决方案：** 使用图标代替文字标题
- 创建 `create_status_icon()` 函数，生成圆点图标（白色/黄色/红色）
- 将 `button.setTitle_("AG")` 改为 `button.setImage_(self.icon_normal)`
- 根据告警等级动态切换图标颜色

**验证方法：**
1. 创建 bar_test17.py（使用 setTitle_）和 bar_test18.py（使用 setImage_）对比测试
2. 确认 System Events 能识别到菜单栏项，但文字版本视觉上不可见
3. 图标版本可以正常显示并响应点击
