...
# [CN] # 网络速度计算 - 保存上一次的统计数据
_last_net_io = None
_last_net_time = None

# [CN] # 磁盘 I/O 速度计算 - 保存上一次的统计数据
_last_disk_io = None
...
class Metrics:
    ...
    # CPU
    cpu_percent: float
    # [CN] cpu_user_percent: float = 0.0      # 用户态 CPU
    # [CN] cpu_system_percent: float = 0.0    # 系统态 CPU
    # [CN] cpu_idle_percent: float = 0.0      # 空闲 CPU
    # [CN] # 磁盘 I/O (KB/s)
...
def _get_memory_from_vm_stat() -> tuple:
    ...
    try:
        ...
        # [CN] # Total（从 psutil 获取，更准确）
        total_gb = psutil.virtual_memory().total / (1024**3)

        # Available = Free + Inactive + Speculative
        # [CN] # （Inactive 可以被快速回收，Speculative 是预读的页面）
        available_gb = free_gb + inactive_gb + speculative_gb

        # [CN] # Used（物理内存实际占用）= Wired + Active + Inactive + Compressor
        # [CN] # 注意：这里用 Compressor（压缩后占用的物理空间），不是 Compressed（原始大小）
        physical_used_gb = wired_gb + active_gb + inactive_gb + compressor_gb

        # [CN] # Percent（内存压力）= (Total - Available) / Total
        # [CN] # 这个百分比反映"还剩多少可用"，不会超过 100%
...
def collect_metrics() -> Metrics:
    ...
    # [CN] # 修复磁盘百分比：macOS APFS 中 disk.used 不包含快照/预留空间
    # [CN] # 正确算法: (total - free) / total，反映用户实际可用空间占比
    ...
    return Metrics(
        ...
        # [CN] disk_used_gb=_gb(disk.total - disk.free),  # 修正：total-free 才是实际占用
