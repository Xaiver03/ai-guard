...
def _ensure_cache():
    ...
    # [CN] logger.info("首次加载 Claude 使用数据到缓存...")
    # TODO: Translate this log message
    _rebuild_cache()
...
def _rebuild_cache(project: Optional[str] = None, source: Optional[str] = None):
    ...
    with _rebuild_lock:
        ...
        # [CN] logger.info(f"缓存已更新: {len(daily_data)} 天, {len(hourly_data)} 小时, 覆盖率: {coverage['coverage_percent']}%")
...
async def get_model_stats(
    ...
    try:
        ...
        logger.error(f"GetModelStatisticsFailure: {e}")
...
async def update_pricing(pricing_data: dict):
    ...
    try:
        ...
        if 'model' in pricing_data:
            # SingleModelFormat: {"model": "xxx", "input": 1.0, ...}
            ...
            # BatchFormat: {"model1": {"input_price": 1.0, ...}, ...}
...
async def delete_pricing(model: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400,
                # [CN] detail="无法删除内置默认定价"
            )

        # [CN] # 后台异步重建缓存
        threading.Thread(target=_rebuild_cache, daemon=True).start()

        # [CN] return {"status": "success", "message": f"已删除 {model} 的定价覆盖"}
    ...
    try:
        with _rebuild_lock:
            ...
            if entries:
                ...
                cache.save_coverage(coverage)
                cache.set_last_update_time(datetime.now().isoformat())
                # [CN] logger.info(f"刷新完成: {len(daily_data)} 天, {len(hourly_data)} 小时")
        ...
        logger.error(f"RefreshDataFailure: {e}")
        raise HTTPException(status_code=500, detail="RefreshDataFailure")
