...
async def remove_process_name(name: str):
    ...
    if not success:
        raise HTTPException(status_code=404, detail="进程名不在白名单中")
    return {"success": True, "message": f"已将 {name} 从白名单移除"}
...
async def remove_command_keyword(keyword: str):
    ...
    if not success:
        raise HTTPException(status_code=404, detail="关键字不在白名单中")
    return {"success": True, "message": f"已将关键字 {keyword} 从白名单移除"}
